...
export default function InlineSVG({ name, className, onClick }) {
  ...
  const file = useMemo(() => `/svg/${name}.svg`, [name]);

  useEffect(() => {
    let alive = true;
    fetch(file)
      .then(r => { if (!r.ok) throw new Error('SVG not found'); return r.text(); })
  ...
  return (
    <div className={className} onClick={onClick} dangerouslySetInnerHTML={{ __html: markup || '' }} />
  );
}
